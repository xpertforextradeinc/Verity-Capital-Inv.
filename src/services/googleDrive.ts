...
export class GoogleDriveService {
  ...
  async listFiles(options: {
    ...
    searchTerm?: string;
    filterType?: 'all' | 'verity_capital_inv' | 'sheets' | 'docs' | 'folders' | 'starred' | 'trash';
    pageToken?: string;
    pageSize?: number;
    ...
    } else if (options.filterType === 'verity_capital_inv') {
      queryClauses.push("(name contains 'Verity-Capital Inv' or description contains 'Verity-Capital Inv' or properties has { key='app' and value='verity_capital_inv' })");
    }
  ...
  async createFolder(name: string, parentId?: string): Promise<DriveFile> {
    ...
    const metadata: Record<string, any> = {
      ...
      properties: { app: 'verity_capital_inv' },
    };
  ...
  /**
   * Find or create the default "Verity-Capital Inv Reports" folder
   */
  async getOrCreateVerityFolder(): Promise<string> {
    const headers = await this.getAuthHeaders();
    const searchParams = new URLSearchParams({
      q: "name = 'Verity-Capital Inv Reports' and mimeType = 'application/vnd.google-apps.folder' and trashed = false",
    ...
    const newFolder = await this.createFolder('Verity-Capital Inv Reports');
    return newFolder.id;
  }
  ...
  async uploadFile(options: {
    ...
    const metadata: Record<string, any> = {
      ...
      description: options.description || 'Generated by Verity-Capital Inv Simulator',
      properties: { app: 'verity_capital_inv' },
    };
  ...
  async exportPortfolioSnapshot(portfolio: Portfolio | null, positions: Position[]): Promise<DriveFile> {
    const folderId = await this.getOrCreateVerityFolder();
    const now = new Date();
    const dateStr = now.toISOString().replace(/[:.]/g, '-').slice(0, 19);
    const fileName = `Verity_Capital_Inv_Portfolio_Statement_${dateStr}.csv`;

    const csvRows: string[] = [
      '# VERITY-CAPITAL INV EXCHANGE SIMULATED PORTFOLIO STATEMENT',
    ...
    return this.uploadFile({
      ...
      content,
      parentId: folderId,
      description: `Verity-Capital Inv simulated portfolio valuation and holdings snapshot generated on ${now.toLocaleString()}`,
    });
  }
  ...
  async exportOrdersLedger(orders: Order[]): Promise<DriveFile> {
    ...
    const now = new Date();
    const dateStr = now.toISOString().replace(/[:.]/g, '-').slice(0, 19);
    const fileName = `Verity_Capital_Inv_Order_Executions_${dateStr}.csv`;

    const csvRows: string[] = [
      '# VERITY-CAPITAL INV EXCHANGE SIMULATED ORDER LEDGER',
      `# Generated At: ${now.toISOString()}`,
      '',
    ...
    return this.uploadFile({
      ...
      content,
      parentId: folderId,
      description: `Verity-Capital Inv simulated order execution and limit ledger report generated on ${now.toLocaleString()}`,
    });
  }
  ...
  async exportAiInsightsBrief(insights: AiInsight[]): Promise<DriveFile> {
    ...
    const now = new Date();
    const dateStr = now.toISOString().replace(/[:.]/g, '-').slice(0, 19);
    const fileName = `Verity_Capital_Inv_AI_Market_Research_${dateStr}.md`;

    const mdLines: string[] = [
      '# Verity-Capital Inv — AI Market Research & Insights Brief',
    ...
    return this.uploadFile({
      ...
      content,
      parentId: folderId,
      description: `Verity-Capital Inv AI market synthesis report for ${insights.length} securities`,
    });
  }
}
